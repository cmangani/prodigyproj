...
class Database(object):
    ...
    def get_dataset(self, name, default=None, session=False):
        ...
        if session == True:
            query = query.where(Dataset.session == True)
    ...
    def drop_dataset(self, name):
        ...
        with self.db.atomic():
            ...
            original_example_ids = [l.example.id for l in links]
            log("DB: Found {} examples to unlink".format(len(original_example_ids)))
            ...
            link_examples = Link.select().where(
                Link.example_id << original_example_ids,
            )
            if len(link_examples) == 0:
                # All the original links were deleted, so the delete list is
                # original_example_ids
                to_delete_example_ids = original_example_ids
                log("DB: All example links were removed")
            else:
                ...
                to_delete_example_ids = [k for k, v in link_counts.items() if v == 0]
                ...
                log("DB: {} out of {} examples with 0 ref-count".format(len(to_delete_example_ids), len(link_examples)))
            ...
            if len(to_delete_example_ids) > 0:
                ...
                log("DB: Deleted {} Examples".format(len(to_delete_example_ids)))
            self.db.commit()
